...
## Setting ##
check=">"     ##  > or <
value=0.01    ##  `>0.01` is means if the z of atom >0.01, the atom will be selected to relax(add T T T),nor add F F F
file_name=POSCAR-0
out_file_name=POSCAR-m
## Done ##
if [ $# -eq 2 ]; then
check=$1
value=$2
...
check_value=`echo "${z}${check}${value}" | bc` 
